...
export class UserService
{
    ...
    get(): Observable<User>
    {
        ...
        return this._httpClient.get<any>(`http://10.252.246.174:8081/api/users/${localUser.id}`).pipe(
            map(response => {
                return {
                    id: response.id,
    ...
    update(user: User): Observable<any>
    {
        ...
        return this._httpClient.put<any>(`http://10.252.246.174:8081/api/users/${localUser.id}`, updateData).pipe(
            tap(() => {
                this.get().subscribe();
            })
        );
