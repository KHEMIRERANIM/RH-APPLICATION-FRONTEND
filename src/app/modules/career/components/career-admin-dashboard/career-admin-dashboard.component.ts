...
export class CareerAdminDashboardComponent implements OnInit, OnDestroy {
  ...
  // === SOLUTION AU PROBLÈME Math.round dans le template ===
  Math = Math;   // Cela permet d'utiliser Math.round() directement dans le HTML
  ...
  loadStats(): void {
    ...
    this.http.get<any[]>('http://localhost:8081/api/users', { headers: this.getHeaders() })
      .pipe(catchError(() => of([])))
      .subscribe(users => {
        this.totalEmployees = users.filter(u => u.role === 'EMPLOYE').length;
      });
  }
